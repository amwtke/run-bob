...
#
# Options:
#   --project-dir <path>  Store session files under <path>/.bob/model-review/
...
if [[ "$FOREGROUND" == "true" ]]; then
  echo "$$" > "$PID_FILE"
  env BOB_REVIEW_DIR="$SESSION_DIR" BOB_REVIEW_HOST="$BIND_HOST" BOB_REVIEW_URL_HOST="$URL_HOST" BOB_REVIEW_OWNER_PID="$OWNER_PID" node server.cjs
  exit $?
fi

# Start server, capturing output to log file
# Use nohup to survive shell exit; disown to remove from job table
nohup env BOB_REVIEW_DIR="$SESSION_DIR" BOB_REVIEW_HOST="$BIND_HOST" BOB_REVIEW_URL_HOST="$URL_HOST" BOB_REVIEW_OWNER_PID="$OWNER_PID" node server.cjs > "$LOG_FILE" 2>&1 &
SERVER_PID=$!
