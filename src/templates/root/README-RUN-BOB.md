...
本 harness 强制遵守 4 环向内依赖。详见 CLAUDE.md R0-R13。
...
事务 / 装配 / 事件订阅都收敛在 ring 3-4。选择 Java/Spring 落地时,**唯一的 `@Transactional`** 出现在
`shared.framework.transaction.TransactionalUseCaseDecorator`。该可选 shared 骨架只由 `run-bob init --with-java` 安装。
...
### 可选 ArchUnit 守卫(`--with-java`)

只有运行 `run-bob init --with-java` 才会生成
`src/test/java/architecture/CleanArchitectureTest.java`。若项目选择了这套 Java skeleton,
该文件就是 4 环规则的机械执法者:**请勿删除其中已有规则**;可在末尾追加项目特定规则,
并在 CI 中将它设为合并门槛。未选择 `--with-java` 的项目不应假定该测试存在。
...
本 harness 通过**8 Bob 工作流 skills + 辅助 `visual-md` skill + 两份锚点文档**,以及仅在 `run-bob init --with-java` 时安装的**可选 shared Java 骨架与 ArchUnit 守卫**,把 Bob 4 环架构原则
...
│   ├── bob-nfr/SKILL.md              # ⚖️ NFR 评审
│   └── visual-md/SKILL.md            # 🖼 辅助可视化(含 scripts/)
├── CLAUDE.md                          # 🛡 项目级硬约束(R0-R13,user-owned)
├── ARCHITECTURE.md                    # 📘 4 环架构 SSoT(user-owned)
├── README-RUN-BOB.md                  # 📖 本文档
├── .gitignore                         # run-bob 受管区块(除非使用 --no-gitignore)
└── docs/
    ├── bob/                           # Bob 中间产物
    ├── specs/                         # bob-spec 输出
    └── compliance/
...
这份文档是 Bob 项目的规则文档。Claude Code 按其项目约定读取它;Codex 工作流会显式读取 `CLAUDE.md`;不能假定 Codex 自动加载它。本 harness 不生成或管理额外的代理规则文件。它定义:
...
---
*Generated by run-bob. 如需更新受管 harness 资产,先运行 `run-bob upgrade --dry-run`,确认后再运行 `run-bob upgrade`。*
