...
- Codex：使用当前可用的 PDF/文档读取能力读取同一输入，并按相同粒度抽取规则。
- 当前所选宿主无法读取输入文档时，立即停止并清楚报告该输入无法由当前宿主读取；不得切换宿主或猜测规则。
- 规则抽取、diff 校验与报告生成均由当前宿主代理执行，不引入新二进制或外部转换进程。
...
| 宿主 | 后缀 | 读取方式 |
|---|---|---|
| Claude Code | `.pdf`、`.docx` | 使用 Read 工具的 `pages` 参数分批读取(每次 5-10 页),逐批抽取规则 |
| Codex | `.pdf`、`.docx` | 使用当前可用的 PDF/文档读取能力,按相同的 5-10 页粒度分批抽取规则 |
| 任一宿主 | `.md`、`.txt`、`.markdown` | 使用当前宿主等价的文本读取能力,一次性读取全文 |
| 任一宿主 | 其他不支持格式 | 跳过该输入,并在最终报告里标注“格式不支持” |
| 任一宿主 | 已支持格式但当前宿主无法读取、文件损坏或权限拒绝 | 立即停止并报告具体输入与原因；不得切换宿主、回退到另一 skill 根或猜测规则 |

### 1.2 抽取规则

当前宿主代理在原文里识别以下结构(以阿里嵩山版为典型):
...
- 源文件不可读(损坏 / 权限)→ 立即停止整个运行；报告具体源文件路径与原因；不得切换宿主、不得猜测规则；lock 不记录该文件；下次重跑时仍按"漂移"处理
...
- **可机械检测**(命名、空格、关键字位置等)→ 当前宿主代理用模式匹配 / 正则检视 diff 文本
- **需语义判断**(异常处理思路、并发设计、安全实践)→ 当前宿主代理逐文件逐函数检视上下文,判断是否符合规则意图
...
模式规则(PAT-*)套用同一三类:**违反** = 声明了模式但 diff 找不到对应结构(接口缺失 / 只 1 实现 / Context 直接耦合具体类),按该 PAT 行在 §9.5 声明的档位(采纳时默认【强制】);**待量化** = §9.5 没给可观察痕迹或太模糊,建议回 `/bob-spec` 补痕迹;**豁免** = spec §10 开放问题显式注明降级理由。
...
- **当前宿主代理是唯一执行器**:PDF → md、规则抽取、diff 校验全在当前宿主代理内部,不引入新二进制 / 不调外部进程
